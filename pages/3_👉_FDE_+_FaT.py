...
# Main app
st.write('# CrysX-DEMO: Frozen Density Embedding (FDE) with Freeze-and-Thaw (FaT) cycles')
st.write('Freeze-and-Thaw refers to the process where the roles of the active (A) and environment (B) subsystems are interchanged iterativley, until either the max cycles is reached or the energies/densities are converged. You can perform FDE calculations on the already available small test systems or use your own. NOTE: Calculations can only be performed for systems with less than 50 basis functions due to limited compute resources on the server where the web app is freely hosted.')
st.write('FDE with FaT ensures that the total energy is minimized wrt both A and B subsystem densities. As a result it is also minimized wrt the total density.')
st.write('Therefore, FDE + FaT can also be referred to as subsystem DFT. In theory, it improves upon FDE where an approximation is used for the frozen environment density (usually the isolated KS-DFT density of the environment is used).')
st.write('The partitioning of the total density in subsystem DFT is not unique, however, the FDE+FaT procedure results in unique subsystem densities as an artifact of the approximate non-additive kinetic energy density functionals.')
...
fat_max_cycles = st.slider('No. of cycles to be used for FaT', 1, 5, 3)

# show_scf_summary =  st.checkbox('Show SCF Summary', key='scf_summary')

col1, col2, col3 = st.columns([1,1,1])
if col2.button('Run FDE + FaT calculation'):
    if isDemo:
        if molTot.nao>=50:
    ...
    for iFaT in range(1,fat_max_cycles+1):
        ...
        with st.spinner('##### Running a FDE calculation on the subsystem A using the frozen density of B from previous step...'):
            ...
            if isFDEconverged:
                st.success('###### FDE energy of the subsystem A =   **'+ str(energyA_FDE)+'**'+'  a.u.', icon = '✅')
        ...
        with st.spinner('Running a FDE calculation on the subsystem B using the frozen density of A from previous step...'):
            # if isDF:
            ...
            if isFDEconverged:
                st.success('###### FDE energy of the subsystem B =   **'+ str(energyB_FDE)+'**'+'  a.u.', icon = '✅')
                ...
                if iFaT==fat_max_cycles:
                    ...
                    with st.expander('See the potential matrices'):
                        ...
                        st.write('#### Non-additive kinetic potential matrix')
                        st.latex(r'T^\mathrm{nadd}_{\mu \nu} = \left<\mu \right | \frac{\delta T_{\mathrm{s}}^{\mathrm{nadd}}\left[\rho^{\mathrm{A}}, \rho^{\mathrm{B}}\right]}{\delta \rho^{\mathrm{A}}(\boldsymbol{r})} \left | \nu \right>')
                        st.write(pot_matrices_B['nadd_KE_pot'])
                    with st.expander('See the orbital info and density matrix'):
                        st.write('#### MO Energy Info')
                        tmp_list = []
            ...
            st.warning('##### Energy of the total system from FDE =   **'+ str(energyTot_FDE)+'**'+'  a.u.')

            
    # st.balloons()
        
    st.write('The final energy of the total system from FDE + FaT')
    st.latex('E_\mathrm{Tot} = E_\mathrm{A}+E_\mathrm{B}+E_\mathrm{int}')
    # energyTot_FDE = energyA_FDE + energyB_FDE - E_intAB
    st.success('##### Energy of the total system from FDE + FaT =   **'+ str(energyTot_FDE)+'**'+'  a.u.', icon = '✅')
    ...
    st.info('##### *Error* ($E_\mathrm{DFT} - E_\mathrm{FDE}$) = '+str(np.format_float_scientific(energyTot-energyTot_FDE, exp_digits=2, precision=4))+'  a.u.')
